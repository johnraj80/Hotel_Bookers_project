...
import { fetchPendingHotels, verifyHotel } from '../../features/admin/adminSlice';

const HotelVerification = () => {
  const dispatch = useDispatch();
  const pendingHotels = useSelector((state) => state.admin.pendingHotels);

  useEffect(() => {
    dispatch(fetchPendingHotels());
  }, [dispatch]);

  const handleVerification = (hotelId, action) => {
    dispatch(verifyHotel({ hotelId, action }));
  };
